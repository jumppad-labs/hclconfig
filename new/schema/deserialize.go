...
/*
tp, err := parseType(p.Type)
		if err != nil {
			return nil, err
		}

		if p.Properties != nil {
			nestedAttribute, err := parseAttribute(p)
			if err != nil {
				return nil, err
			}
			...
			nf := reflect.StructField{
				Name: p.Name,
				Type: reflect.TypeOf(nestedAttribute),
				Tag:  reflect.StructTag(p.Tags),
			}
			...
			if tp.Slice {
				innerType := reflect.TypeOf(tp.Type)

				if tp.InnerPointer {
					innerType = reflect.PointerTo(reflect.TypeOf(tp.Type))
				}
				...
				keyType := reflect.TypeOf(tp.MapKey)
				innerType := reflect.TypeOf(tp.Type)

...
func TestReadResourceFromFileAtLocation(t *testing.T) {
	c, err := ReadFileLocation("./internal/test_fixtures/config/single/container.xcl", 13, 1, 32, 2)
...
func TestLineFromFileAtLocation(t *testing.T) {
	c, err := ReadFileLocation("./internal/test_fixtures/config/single/container.xcl", 13, 1, 13, 30)
	require.NoError(t, err)
